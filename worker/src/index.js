/* ==========================================================================
   Rewards indexer — Cloudflare Worker
   --------------------------------------------------------------------------
...
async function loadState(env) {
  const raw = await env.REWARDS.get(STATE_KEY, 'json');
  if (!raw) return { cursor: START_BLOCK, totals: {}, updatedAt: null, lastError: null };
  return raw;
}

async function saveState(env, state) {
  await env.REWARDS.put(STATE_KEY, JSON.stringify(state));
}
...
async function loadBalances(env) {
  return (await env.REWARDS.get(BALANCES_KEY, 'json')) || {};
}
...
/** Reward-token price in USD, or null. Never fatal — the totals matter more. */
...
async function sync(env, ctx) {
  ...
  if (state.cursor <= head) {
    ...
    if (balances) await env.REWARDS.put(BALANCES_KEY, JSON.stringify(balances));
  }
...
function present(state, kex) {
  ...
  return {
    ...
    // Same fees expressed in the reward token, for the unit beside the dollars.
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/reset' && request.method === 'POST') {
      ...
      await env.REWARDS.delete(STATE_KEY);
      await env.REWARDS.delete(BALANCES_KEY);
