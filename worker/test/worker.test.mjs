...
/* While worker/src/config.js still carries nulls, the chain-shaped tests have
   no addresses to run against and no scale to assert in. They SKIP rather
   than fail: a template that ships a red suite teaches whoever copies it to
   stop reading the suite, and then a real failure arrives in a colour nobody
   looks at any more. Filling the config in turns them all back on, and the
   scale assertions below are what catch a wrong decimals() reading. */
import { MISSING } from '../src/config.js';
const incomplete = MISSING.length
  ? `worker/src/config.js is incomplete: ${MISSING.join(', ')}`
  : false;
const chainTest = (name, fn) => test(name, { skip: incomplete }, fn);
...
// A stand-in while START_BLOCK is null, so the fixtures below can still be
// built for a suite that is about to skip them anyway.
const START = START_BLOCK ?? 1_000_000;
...
chainTest('cron sync banks totals, then a later run resumes without re-counting', async () => {
...
chainTest('fees track the rewards contract, not the platform-wide fee locker', async () => {
...
chainTest('holder payout strips the protocol cut', async () => {
...
chainTest('serves CORS and the documented shape', async () => {
  const mod = (await import('../src/index.js')).default;
...
chainTest('reset requires the admin token', async () => {
  const mod = (await import('../src/index.js')).default;
...
chainTest('a failing RPC is recorded, not thrown, and totals are kept', async () => {
  const mod = (await import('../src/index.js')).default;
...
chainTest('counts holders from transfers, across a resumed backfill', async () => {
  const mod = (await import('../src/index.js')).default;
...
chainTest('withholds the holder count until the backfill has finished', async () => {
  const mod = (await import('../src/index.js')).default;
