...
import {
  ...
  holderPayout, DEXSCREENER_KEX_TOKEN, CONTRACTS, MISSING,
...
async function main() {
  /* Refuse rather than run half-configured. A scan with no rewardsIndex sums
     nothing, and a scan with no START_BLOCK starts at genesis — both end in
     data/rewards.json full of nulls, published on a schedule and
     indistinguishable on the page from a site that is broken. Exiting clean
     leaves the committed file untouched. */
  if (MISSING.length) {
    console.log('worker/src/config.js is incomplete — not scanning.');
    console.log('  still null: ' + MISSING.join(', '));
